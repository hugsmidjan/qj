...
// Returns the original if nothing changed.
const objectClean = <T extends object, N extends boolean = false>(original: T, alsoNull?: N): Optionaled<T, N> => {
  let deleted;
...
// Returns the original if nothing changed.
const objectOnly = <T extends object, K extends keyof T>(original: T, keys: Array<K>): Pick<T, K> => {
  let extra;
  const clone = _createEmpty(original);
  for (const key in original) {
    if ( hasOwnProperty.call(original, key) ) {
...
// Returns the original if nothing changed.
const objectWithout = <T extends object, K extends keyof T>(original: T, keys: Array<K>): Omit<T, K> => {
  let clone;
...
const objectReplace = (original: object, replacement: object, customSameCheck?: SameChecker) =>
