...
o.spec('Memoize', () => {
  ...
  o('Memoizes (...arg) functions with multiArg param', () => {
    const _decimalize = o.spy((...args: Array<number>) =>
      args.reverse().reduce((acc, arg, i) => acc + arg * Math.pow(10, i), 0)
    );
  ...
  o('Memoizes (n, ...arg) functions with multiArg', () => {
    const _decimalize = o.spy(
      (n: number, ...args: Array<number>) =>
        n * Math.pow(10, args.length) +
        args.reverse().reduce((acc, arg, i) => acc + arg * Math.pow(10, i), 0)
    );
    const decimalize = memoize(_decimalize, true);
    decimalize(1, 2, 3);
