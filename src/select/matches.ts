...
function matches<S extends keyof HTMLElementTagNameMap>(
	selectors: S,
	elm: _Element
): elm is HTMLElementTagNameMap[S];
function matches<S extends keyof SVGElementTagNameMap>(
	selectors: S,
	elm: _Element
): elm is SVGElementTagNameMap[S];
...
function matches(selectors: string, elm: _Element): boolean {
	if (_matcher == null) {
		_matcher =
			elm.matches || elm.msMatchesSelector || elm.webkitMatchesSelector || (() => false);
	}
